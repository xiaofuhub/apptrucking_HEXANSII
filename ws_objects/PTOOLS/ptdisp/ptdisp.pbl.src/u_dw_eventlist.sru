HA$PBExportHeader$u_dw_eventlist.sru
forward
...
IF ll_Return = 0 AND THIS.RowCount ( ) > 0 THEN // 0 = Continue processing
	...
	CHOOSE CASE Key 
		...
		IF isValid( lnv_shipment ) THEN
			IF lnv_shipment.of_isBilled( ) THEN
				ls_privsFunction = appeon_constant.cs_ModifyBilledShip
			ELSE  
		...
		END IF
		//////////////////////////
		IF gnv_app.of_getprivsmanager( ).of_Getuserpermissionfromfn( ls_privsFunction, lnv_Shipment ) = appeon_constant.ci_False THEN
			ll_Return = 1
		ELSE
