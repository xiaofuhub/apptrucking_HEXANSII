HA$PBExportHeader$u_tabpage_unassignedamounts.sru
$PBExportComments$UnassignedAmounts (Tab Page from PBL map PTSetl) //@(*)[84662165|1569]
forward
global type u_tabpage_unassignedamounts from u_tabpage_amounts
end type
end forward

global type u_tabpage_unassignedamounts from u_tabpage_amounts
long backcolor = 12632256
string text = "UnassignedAmounts"
end type
global u_tabpage_unassignedamounts u_tabpage_unassignedamounts
...
on u_tabpage_unassignedamounts.create
call super::create
end on

on u_tabpage_unassignedamounts.destroy
call super::destroy
end on

type dw_amounts from u_tabpage_amounts`dw_amounts within u_tabpage_unassignedamounts
end type
	...
	CASE "ratecodename"
			...
			if parent.of_autorate(lnva_ratedata, data, this.object.amountowed_quantity[row], this.object.billtoid[row], &
				...
				if lnva_ratedata[1].of_getrate() > 0 then
					if lnva_ratedata[1].of_Getratetype() = n_cst_bso_rating.cs_RateUnit_Code_Flat then
...
type dw_amountdetail from u_tabpage_amounts`dw_amountdetail within u_tabpage_unassignedamounts
...
event dw_amountdetail::buttonclicked;call super::buttonclicked;n_cst_msg	lnv_msg
decimal		lc_quantity
s_parm		lstr_parm
