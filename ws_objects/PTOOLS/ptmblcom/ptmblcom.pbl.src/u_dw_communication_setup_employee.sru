HA$PBExportHeader$u_dw_communication_setup_employee.sru
forward
global type u_dw_communication_setup_employee from u_dw
end type
end forward
...
global u_dw_communication_setup_employee u_dw_communication_setup_employee
...
IF lnv_LicenseMgr.of_HasCommunicationsLicense ( ) THEN
	IF Len ( Trim ( ls_ValueList ) ) > 0 THEN
		ls_ValueList +=  "/"
	END IF
	ls_ValueList += n_cst_Constants.cs_communicationdevice_Nextel + "~t" + n_cst_Constants.cs_communicationdevice_Nextel
END IF
...
inv_Sort.of_SetExclude ( { "employees_em_fn" , "employees_em_mn" , "employees_em_ln" , "employeeid" , "equipmentid" } ) 
...
inv_Filter.of_SetExclude ( { "employees_em_fn" , "employees_em_mn" , "employees_em_ln" , "employeeid" , "equipmentid" } ) 


RETURN li_Return
end event

on u_dw_communication_setup_employee.create
end on

on u_dw_communication_setup_employee.destroy
end on
