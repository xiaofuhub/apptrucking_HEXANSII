HA$PBExportHeader$n_cst_companies.sru
forward
...
if ll_foundrow > 0 then
	...
	case "BILLING!", "BILLING_OR_WARNING!"
		...
		case "F" //'Accounts Payable' First
			...
			if len(ls_bill_attn_text) > 0 then ls_work += " $$HEX2$$13202000$$ENDHEX$$" + ls_bill_attn_text
		case "L" //'Accounts Payable' Last
			ls_work += "ATTN: "
			if len(ls_bill_attn_text) > 0 then ls_work += ls_bill_attn_text + " $$HEX2$$13202000$$ENDHEX$$"
