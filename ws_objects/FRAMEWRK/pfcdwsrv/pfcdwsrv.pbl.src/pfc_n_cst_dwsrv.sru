...
//
//	Description:  
//	Returns the formatted (including formats, editmasks and display
//	values) text of any column on a datawindow, regardless of the 
//	column's datatype.  
//
//////////////////////////////////////////////////////////////////////////////
...
CHOOSE CASE Left ( ls_coltype , 5 )
		...
		CASE ELSE 					//  MUST BE A COMPUTED COLUMN
			IF idw_Requestor.Describe ( as_column + ".Type" ) = "compute" THEN 
				ls_computeexp = Trim(idw_Requestor.Describe ( as_column + ".Expression"))
				IF	Left (ls_computeexp,2)='~'~"' And Right(ls_computeexp,2)='~"~'' Then
				ElseIf (Left (ls_computeexp,1)<>'~"' And Right(ls_computeexp,1)<>'~"') Then
					ls_computeexp = '"' + ls_computeexp + '"'
				End If
				ls_evaluateexp = 'Evaluate(' + ls_computeexp + ', ' + string (al_row) + ')'					
