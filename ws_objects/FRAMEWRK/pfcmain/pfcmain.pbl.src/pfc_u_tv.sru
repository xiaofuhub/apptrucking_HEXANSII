...
if istr_DS[1].s_DataWindow = "" then
	return -2
end if
...
If ids_Buffer.of_SetTransObject(istr_DS[1].tr_obj) <> 1 Then Return -5
...
If istr_DS[ai_Level].ds_obj.of_SetTransObject(atr_obj) <> 1 Then 
...
If istr_DS[ai_Level].ds_obj.Describe(as_LabelColumn + ".Band") = "!" Then 
	istr_DS[ai_Level].s_DataWindow = ""
	Destroy istr_DS[ai_Level].ds_obj
	Return -4
End if
...
If al_Parent > 0 Then
	...
	// Need to connect ok before we do a retrieve
	If ids_Buffer.of_SetTransObject(istr_DS[li_Index].tr_obj) <> 1 Then Return -5
...
// Append the rows to the level's DataStore
ll_Rows = istr_DS[li_Index].ds_obj.RowCount()
ids_Buffer.RowsCopy(1, ll_NewRows, Primary!, istr_DS[li_Index].ds_obj, (ll_Rows + 1), Primary!)

// Clear the status flags for the new rows
For ll_Cnt = (ll_Rows + 1) To (ll_Rows + ll_NewRows)
	istr_DS[li_Index].ds_obj.SetItemStatus(ll_Cnt, 0, Primary!, DataModified!)
	istr_DS[li_Index].ds_obj.SetItemStatus(ll_Cnt, 0, Primary!, NotModified!)
Next
